...
with the specified player and list of cards.
	...
	public boolean isValid() {
		if ( this.size() == 5 ) {
			...
			//single in the first card
			if ( (card2.rank == card3.rank) & (card3.rank == card4.rank) ) { 
				if ( (card4.rank == card5.rank) & (card1.rank != card2.rank) ) {
					return true;
				}
				else return false;
			}
			//single in the last card
			else if ( (card1.rank == card2.rank) & (card2.rank == card3.rank) ) { 
				if ( (card3.rank == card4.rank) & (card4.rank != card5.rank) ) {
					return true;
				}
				else return false;
			}
